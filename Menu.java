...
public class Menu
{
	...
	private static void addLecture()
	{
      ...
      //add event menu
		try
		{
			...
			inHour =  sc.nextInt();
			inMinute = sc.nextInt();
			event = new EventItems(inUnit, inLectureHall, inDay, inMonth, 
                                      inYear, inHour,inMinute);
         updateArray(event.getISchedule());
		}
	...
	private static void addTutorial()
	{
      ...
      //add event menu
		try
		{
			...
			inHour =  sc.nextInt();
			inMinute = sc.nextInt();
         event = new EventItems(inUnit, inClassroom, inDay,
   ...
   //User is returned to the main menu if an exception is caught
